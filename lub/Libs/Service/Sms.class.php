...
class Sms extends \Libs\System\Service {
	...
	*/
	function order_msg($info,$type = '1'){
		 ...
		 $num = $info['num'];
		 $sn = substr($info['sn'],0,1).substr($info['sn'],-6);//10位$remark订单详情
		 $remark = $info['remark'];
		 ...
		 switch ($type) {
		 	...
		 	case '7':
		 		...
		 		$message = urlencode($title."共".$num."张，其中".$area."。".$channel."。【".$info['product']."】");
		 		break;
		 	case '8':
		 		//分销账号审核
    ...
    */
	...
	private function SAEsendmsg($parameter){
		$conf = cache('Config');
		$accesskey  = $conf['sms_account'];
		$secretkey = $conf['sms_key'];
		$url ="http://sms.bechtech.cn/Api/send/data/json?accesskey=".$accesskey."&secretkey=".$secretkey."&".$parameter;
		$ch = curl_init();
