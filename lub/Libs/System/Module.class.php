...
class Module {
    ...
    public function install($moduleName = '') {
        ...
        if ($config['adaptation']) {
            if (version_compare(LUB_VERSION, $config['adaptation'], '>=') == false) {
    ...
    public function uninstall($moduleName = '') {
        ...
        //删除权限
        M("Manage/Access")->where(array("app" => $moduleName))->delete();
        //移除菜单项和权限项
        M("Manage/Menu")->where(array("app" => $moduleName))->delete();
        //去除对应行为规则
    ...
    private function installMenu($moduleName = '', $file = 'Menu') {
        ...
        $status = D('Manage/Menu')->installModuleMenu($menu, $this->config($moduleName));
        if ($status === true) {
            return true;
        } else {
            $this->error = D('Manage/Menu')->getError()? : '安装菜单项出现错误！';
            return false;
        }
    }
