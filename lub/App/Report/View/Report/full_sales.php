...
<!--Page -->
<form id="pagerForm" data-toggle="ajaxsearch" action="{:U('Report/Report/full_sales',array('menuid'=>$menuid));}" method="post">
...
<div class="visible-print-block">
    <h3 align="center">{$product_id|productName}全员销售(票型)统计(场次)报表</h3>
...
<table class="table table-bordered">
  <thead>
    ...
    <volist name="data" id="channel">
      ...
      <if condition="$l eq '1'">
      <tr>
        <td align="left" colspan="13">渠道商:{$price.channel_id|userName}</td>
      </tr>
      </if>
  ...
  <thead>
    ...
    <?php $price_count = count($channel['price']);?>
      <volist name="channel['price']" id="item" key="k">
        <if condition="$price_count neq '1'">
          <if condition="$k eq '1'">
            <tr class="subtotal" data-num="{$channel.number}" data-money="{$channel.money}" data-moneys="{$channel.moneys}" data-subsidy="{$channel.rebate}">
              <td rowspan="{$price_count}" align="center">{$item.channel_id|userName}</td>
              <td align="center">{$item.priceid|ticketName}</td>
              <td align="right">{$item.price}</td>
              <td align="right">{$item.discount}</td>
              <td align="center">{$item.number}</td>
        ...
        <else />
          <tr  class="subtotal" data-num="{$channel.number}" data-money="{$channel.money}" data-moneys="{$channel.moneys}" data-subsidy="{$channel.rebate}">
            <td align="center">{$item.channel_id|userName}</td>
            <td align="center">{$item.priceid|ticketName}</td>
            <td align="right">{$item.price}</td>
            <td align="right">{$item.discount}</td>
            <td align="center">{$item.number}</td>
