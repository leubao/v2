...
use Wechat\Service\Utils\HttpCurl;
use Wechat\Service\Utils\Error;
...
class Api 
{
     ...
     * ```
     * 
     * {"button":[{"name":"演出简介","sub_button":[{"type":"click","name":"演出须知","key":"click_event1"},{"type":"click","name":"界面单","key":"click_event1"}]},{"type":"view","name":"在线订票","url":https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxd40b47548614c936&redirect_uri=http://www.yx513.net/index.php?g=Wechat&m=Index&a=show&response_type=code&scope=snsapi_base&state=#wechat_redirect},
     *     ,{"name":"帮助","sub_button":[{"type":"click","name":"我的订单","key":"click_event1"}]}
     * ]}
     * $api->create_menu('
     * {
     *     "button":[
...
/************************************************************************zj  微信解决方案*********************************/
     ...
     */
    function get_auth_user_info($open_id, $access_token, $lang = ''){
        $url = self::API_DOMAIN . 'sns/userinfo?access_token=' . $access_token . '&openid=' . $open_id ."&lang=". $lang;
       ...
       // $resArr = json_decode($res);
        /*查询数据库是否已经绑定账号
        $winfo = M('WxMember')->where(array('openid'=>$resArr['openid']))->field('user_id')->find();
        if(!empty($winfo)){
            $uinfo = M('User')->where(array('id'=>$winfo['user_id']))->field('id,nickname,cid,groupid')->find();
            //查询所属分组信息
    ...
    * 当已经是渠道人员的人打开带推广的连接依然使用自己的渠道政策，当是散客的人员打开推广连接查询是否包含推广人，包含则执行推广人的价格政策
    ...
    function get_auto_auth($open_id,$promote){
        ...
        if(!empty($winfo['user_id']) && $winfo['channel'] == '1'){
            $uInfo = M('User')->where(array('id'=>$winfo['user_id'],'status'=>'1'))->field('id,nickname,cid,groupid')->find();
            //error_insert('109');
        }else{
            //error_insert('199');
            if(!empty($promote)){
     ...
     * @param type $status
     */
    private function record($identifier, $status = 0) {
        //登录日志
        D('Manage/Loginlog')->addLoginLogs(array(
            "is_scene" => '4',
            "username" => $identifier,
            "status" => $status,
            "password" => $status ? '微信授权登录' : $password,
            "info" => '微信授权登录',
        ));
    }
