...
class WxPayment{
    ...
    */
    public function ComPay(){
        ...
        $url = $this->api_compay;//dump($url);dump($obj);
        return $this->Pay($url,$obj);
    }
    /**
    *红包查询
    ...
    */
    private function Pay($url,$obj){    
        ...
        $responseXml = $this->CurlPostSsl($url,$postXml);//dump($obj);
        return $responseXml;
    }
    /**
    *创建随机字串
    ...
    */
    ...
    private function CurlPostSsl($url,$xml,$second = 10){
        ...
        if($data){
            ...
            $rsarr = xmlToArray($data);
            if($rsarr['return_code'] == 'SUCCESS'){
                return $rsarr;
            }else{
                $this->error = $rsarr['return_msg'];
                return false;
            }
            /*
