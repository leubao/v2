...
use Wechat\Service\Wxpay\WxPayData;
...
class WxPayApi
{
    ...
    public function __construct($wxpaycfg) {
        ...
        $this->wxpayresults = new WxPayResults($wxpaycfg);
    }
	...
	public function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($this->wxpaycfg->notify_url);//异步通知url
		}

		$inputObj->SetAppid($this->wxpaycfg->appid);//公众账号ID
		$inputObj->SetMch_id($this->wxpaycfg->mchid);//商户号
		$inputObj->SetSub_Appid($this->wxpaycfg->sub_appid);//公众账号ID
		$inputObj->SetSub_Mchid($this->wxpaycfg->sub_mchid);//商户号
	...
	public function notify($callback, &$msg)
	{
		...
		//如果返回成功则验证签名
		try {
			$result = $this->wxpayresults->Init($xml);
	...
	private function reportCostTime($url, $startTimeStamp, $data)
	{
		...
		$objInput = new WxPayReport($this->wxpaycfg);
