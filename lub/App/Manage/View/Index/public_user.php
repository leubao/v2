...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="{:U('Manage/Index/public_user',array('type'=>$type,'ifadd'=>$ifadd));}" method="post">
        <input type="hidden" name="pageCurrent" value="{$currentPage}" />
        <input type="hidden" name="pageSize" value="{$numPerPage}" />
        <input type="hidden" name="ifadd" value="{$ifadd}" />
        <div class="bjui-searchBar">
            <input type="text" value="{$pinfo.name}" name="name" size="10" placeholder="姓名" />&nbsp;
            <input type="text" value="{$pinfo.phone}" name="phone" size="10" placeholder="手机号" />&nbsp;
            <input type="text" value="{$pinfo.legally}" name="legally" size="10" placeholder="导游证号" />&nbsp;
