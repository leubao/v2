...
class MenuModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('is_scene', array(1, 3), '状态值的范围不正确！', 2, 'in'),
    );
    ...
    public function getMenuList() {
        ...
        $data = $this->getTree(0);
        $data = array_merge($changyong, $data ? $data : array());
       // dump($data);
        return $data;
    }
    ...
    public function adminMenu($parentid, $with_self = false) {
        ...
        $result = $this->where(array('parentid' => $parentid, 'status' => 1, 'is_scene' => 1))->order('listorder ASC,id ASC')->select();
        if (empty($result)) {
            $result = array();
        }
        if ($with_self) {
        ...
        //是否超级管理员
        if (\Manage\Service\User::getInstance()->isAdministrator()) {
            //如果角色为 1 直接通过
            return $result;
        }
        $array = array();
    ...
    public function getMenu() {
       ...
       // $info = $this->where(array("id" => $menuid))->getField("id,action,app,controller,icon,parentid,parameter,type,name");
        $find = $this->where(array("parentid" => $menuid, "status" => 1))->order('listorder ASC,id ASC')->getField("id,action,icon,app,controller,width,height,parentid,parameter,type,name,stype,target,is_param");
        $help = $this->where(array("id" => $menuid, "status" => 1))->getField('help');
        /*if ($find) {
    ...
    public function installModuleMenu(array $data, array $config, $parentid = 0) {
        ...
        //默认安装时父级ID
        $defaultMenuParentid = $this->where(array('app' => 'Manage', 'controller' => 'Module', 'action' => 'local'))->getField('id')? : 42;
        //安装模块名称
