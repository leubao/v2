...
class UserController extends ManageBase {
    public function index(){
       ...
       $this->basePage('User',$map,'status DESC,id DESC');
       $this->display();
    }
    //密码重置
    function changepwd(){
    ...
    public function manager() {
        ...
        $User = D('Manage/User')->where($where)->limit($page->firstRow . ',' . $page->listRows)->order(array('status' => 'DESC','id' => 'DESC'))->select();
    ...
    public function delete() {
        ...
        //执行删除
         if (D('Manage/User')->deleteUser($id)) {
            ...
            $this->erun(D('Manage/User')->getError()? : '删除失败！');
        }
    }
    //详情
    function userinfo(){
    ...
    function reset_pwd(){
        ...
        if (1 == $id) {
            $this->erun("该帐号不允许执行此项操作！");
        }
        $data =  array(
            'password' => '123456',
            'id' => $id,
        );
        if (false !== D('Manage/User')->amendManager($data)) {
            $this->srun("更新成功！", array('tabid'=>$this->menuid.MODULE_NAME));
        } else {
            $erun = D('Manage/User')->getError();
            $this->erun($erun ? $erun : '重置失败！');
        }
    }
}