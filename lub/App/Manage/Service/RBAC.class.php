...
class RBAC {
    ...
    static public function authenticate($map = '') {
        ...
        //查询是否有权限
        return D('Manage/Access')->isCompetence($map);
    }
            ...
            if (User::getInstance()->isAdministrator() !== true) {
                ...
                if (!isset($accessList[strtoupper($appName)][strtoupper($controller)][strtoupper(ACTION_NAME)])) {
                    ...
                    if (self::checkLogin() == true) {
                        ...
                        if ($appName == "Manage" && in_array(CONTROLLER_NAME, array("Index", "Main")) && in_array(ACTION_NAME, array("index","index_info"))) {
    ...
    static public function getAccessList($authId) {
        ...
        //检查角色
        $roleinfo = D('Manage/Role')->where(array('id' => $role_id))->find();
        if (empty($roleinfo) || empty($roleinfo['status'])) {
            return false;
        }
        //该角色全部权限
        $access = D('Manage/Access')->getAccessList($role_id);
        $accessList = array();
