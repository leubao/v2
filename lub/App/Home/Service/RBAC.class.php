...
class RBAC {
    ...
    static public function authenticate($map = '') {
        ...
        //是否超级管理员
        if (Partner::getInstance()->isAdministrator() === true) {
            return true;
        }
        //查询是否有权限
        return D('Home/Access')->isCompetence($map);
    }

    //用于检测用户权限的方法,并保存到Session中，登陆成功以后，注册有权限
    static function saveAccessList($authId = null) {
        if (null === $authId)
            $authId = Partner::getInstance()->id;
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if (C('USER_AUTH_TYPE') != 2 && Partner::getInstance()->isAdministrator($authId) !== true)
    ...
    static public function checkLogin() {
        ...
        if (RBAC::checkAccess()) {
            //检查认证识别号
            if (Partner::getInstance()->isLogin() == false) {
                return false;
            }
        }
        return true;
        ...
        if (RBAC::checkAccess()) {
            ...
            if (Partner::getInstance()->isAdministrator() !== true) {
                ...
                if (C('USER_AUTH_TYPE') == 2) {
                    ...
                    //通过数据库进行访问检查
                    $accessList = RBAC::getAccessList(Partner::getInstance()->id);
                } else {
                ...
                if (!isset($accessList[strtoupper($appName)][strtoupper($controller)][strtoupper(ACTION_NAME)])) {
                    ...
                    if (self::checkLogin() == true) {
                    	...
                    	if ($appName == "Home" && in_array(CONTROLLER_NAME, array("Index", "Main")) && in_array(ACTION_NAME, array("index"))) {
    ...
    static public function getAccessList($authId) {
        ...
        //检查角色
        $roleinfo = D('Home/Role')->where(array('id' => $role_id))->find();
        if (empty($roleinfo) || empty($roleinfo['status'])) {
            return false;
        }
        //该角色全部权限
        $access = D('Home/Access')->getAccessList($role_id);
        $accessList = array();
