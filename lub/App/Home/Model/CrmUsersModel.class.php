...
class CrmUsersModel extends Model {
    ...
    public function getuInfo($identifier, $password = NULL) {
        ...
        $map = array();
        //判断是不是UID  2014/09/24 LiRan修改        
        if($password == NULL){
            $map['id'] = $identifier;
        }else{
            //判断是电话号还是用户名
            if (is_int($identifier)) {
                $map['phone'] = $identifier;
            } else {
        ...
        //密码验证
        if (!empty($password) && $this->hashPassword($password, $uInfo['identifyid']) != $uInfo['password']) {
            return false;
        }
        return $uInfo;
    }
