...
class AccessModel extends Model {
    ...
    public function getAccessList($roleid) {
        ...
        //子角色列表
        $child = explode(',', D("Home/Role")->getArrchildid($roleid));
        if (empty($child)) {
            return false;
        }
        //查询出该角色拥有的全部权限列表
    ...
    public function isCompetence($map = '') {
        ...
        if (!is_array($map)) {
            //子角色列表
            $child = explode(',', D("Home/Role")->getArrchildid(\Home\Service\Partner::getInstance()->role_id));
            if (!empty($map)) {
                $map = trim($map, '/');
                $map = explode('/', $map);
                if (empty($map)) {
    ...
    public function getUserAccessList($roleid, $userId = 0) {
        ...
        foreach ($result as $rs) {
            $data = array(
                ...
                'checked' => D('Home/Role')->isCompetence($rs, $roleid, $data) ? true : false,
            );
            $json[] = $data;
        }
        return array();
