    ...
    function seatOrder($param,$plan_id,$type=NULL){
        if(!empty($param) && !empty($plan_id)){
            ...
            if(empty($plan)){
                ...
                $info = M(ucwords($plan['seat_table']))->where(array('seat'=>$param))->field('order_sn,status,print,checktime')->find();
   ...
   * @param 
    ...
    */
    function if_plan($plan_id = null){
        ...
        if($plantime == $datetime){
            if($time >= date('Hi',$plan['starttime'])){
                return false;
            }else{
                return true;
            }
    ...
    */
    function send_sms($plan_id = null){
       //查询销售计划
       $plan = M('Plan')->where(array('id'=>$plan_id))->field('id,seat_table,param')->find();
    ...
    */
    function google_crm($product_id,$crm_id,$gudie){
        ...
        }error_insert($return['groupid']);
        $crmGroup = F('CrmGroup'.$product_id);
    ...
    */
    function pullprice($planid, $type, $area, $scene, $group = null){
        ...
        if($plan['product_type'] == '1'){
            if($scene <> '1'){
                $map = array('area'=>$area,'status'=>1,'type'=>$types,'scene'=>array('like','%'.$scene.'%'),'product_id'=>$plan['product_id'],'group_id'=>$group);
            }else{
                $map = array('area'=>$area,'status'=>1,'type'=>$types,'scene'=>array('like','%'.$scene.'%'),'product_id'=>$plan['product_id']); 
            }
            //获取当前可售数量
