...
function U($url = '', $vars = '', $suffix = true, $domain = true) {
    ...
    if (in_array(C('URL_MODEL'), array(0, 1, 3))) {
        ...
        if (defined('BIND_MODULE')) {
            $homeFile = array('index.php', 'api.php', 'sys.php', 'home.php');
            switch ($_module) {
                case 'Api':
                    ...
                    break;
                case 'Manage':
                    $appUrl = str_replace($homeFile, 'home.php', __APP__);
                    $showModuleName = false;
                    break;
                case 'Home':
                    $appUrl = str_replace($homeFile, 'home.php', __APP__);
    ...
    if (C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if (!empty($var) || ($showModuleName && $module)) {
            $url .= '?';
            //是否显示模块部分
            if (isset($admin_domain) && $var[$varModule] != 'Manage') {
                //如果后台绑定域名访问，且非Manage模块，都要加上模块标识
