...
class ItemReport{
	...
	function report($datetime){
		...
		if($status!=0){
			if(M('ReportData')->where(array('datetime'=>$datetime,'status'=>'1'))->setField('status',0)){
				$status = ItemReport::strip_order($map,$datetime,1);
			}else{
				//失败记录日志，并发送错误短信
	...
	*/
	function strip_order($map, $datetime, $type = '1'){//dump($map);
		...
		if(!empty($list)){
			foreach ($list as $key => $value) {
				...
				$main[] = ItemReport::ticket_type($general,$data['data'],$data['param'],$datetime,$type,$value['product_type']);
				if(!empty($data['child_ticket'])){
					$child[] = ItemReport::ticket_type($general,$data['child_ticket'],$data['param'],$datetime,$type,$value['product_type'],'1',$value['number']);
				}
			}
			if(empty($child)){
				$status = $main;
 	...
 	* @param $number int 数量
 	* @param $is_check int 核算方式 1 结算价 2 净收入 3分销价
 	* return $data
 	* */
 	function ticket_type($general, $seat, $param, $datetime, $type, $product_type, $child = null, $number = null,$is_check = 1){
 		//根据票型归类
 		foreach($seat as $k=>$v){
 		...
 		for ($i=0; $i < count($datalist); $i++) {
 			if(!empty($datalist[$t_type[$i]][0]['priceid'])){
	 			...
	 			$money[$i] = ItemReport::settlement($nums[$i],$t_type[$i],$general['product_id'],$is_check);
	 			if($product_type == '1'){
 		...
 		if($type == '1'){
 			//计划任务拆解
 			return ItemReport::insert_report($datas);
 		}else{
 			//非计划任务拆解
 			return $datas;
 		}
 ...
 /*===============================================^^^^^^^^^^^^每日订单拆解入库^^^^^^^^^^^^^======================================*/	
	...
	*/
	function strip_plan($data){
		foreach ($data as $key => $value) {
			$datas[$key] = ItemReport::plan_ticket_fold($value,$key);
		}
		return $datas;
	}
	/*
	...
	*/
	function ticket_fold($data, $datetime = '', $user_id = null, $channel_id = null, $type = '1'){
		foreach ($data as $key => $value) {
			foreach ($value as $ke => $valu) {
				...
				//获取票型金额
				$money = ItemReport::settlement($num[$key][$valu['price_id']]['num'],$valu['price'],$valu['discount']);
	...
	/*渠道商销售统计 汇总*/
	function channel_fold($data,$is_check = 1){
		//合并渠道商
		$channel = ItemReport::channel_merge($data);
		//单个渠道商合并票型
		$ticket = ItemReport::channel_merge_ticket($channel['channel'],$is_check);
		ksort($ticket);
		return $ticket;
	}
	/*渠道商销售统计 按场次 明细*/
	function channel_plan_fold($data,$is_check = 1){
		//合并渠道商
		$channel = ItemReport::channel_merge($data);//dump($channel);
		ksort($channel['channel']);
		//按计划合并
		foreach ($channel['channel'] as $k => $v) {
			$plan[$k] = ItemReport::plan_merge($v);
		}
		//dump($plan);
		foreach ($plan as $key => $value) {
			$ticket[$key]['plan'] = ItemReport::channel_merge_ticket($value,$is_check);
		}//dump($ticket);
		return $ticket;
	}
	/*数据按渠道商  做合并处理*/
	...
	/*单一渠道商 合并票型*/
	function channel_merge_ticket($data,$is_check){
		//合并渠道商
		//dump($data);
		foreach ($data as $key => $value) {
			$ticket[$key] = ItemReport::plan_ticket_folds($value,1,$is_check);
	...
	*/
	function level_fold($data){
		foreach ($data as $ke => $valu) {
			//判断当前渠道商是否是一级渠道商
			if(ItemReport::if_level($valu['channel_id'],$valu['product_id'])){
				$list[] = $valu;
			}else{
				//将二三级渠道商全部归属到一级渠道商
	...
	function channel_ticket_fold($data, $datetime,$channel = null){
		foreach ($data as $key => $value) {
			foreach ($value as $ke => $valu) {
				...
				//获取票型金额
				$money = ItemReport::settlement($num[$key][$valu['price_id']][$channel_id]['num'],$valu['price_id'],$valu['product_id']);
	...
	function plan_ticket_folds($data,$work = '1',$is_check = 1){
		...
		foreach ($data as $k => $valu) {
			...
			if($work == '1'){
				...
				$money = ItemReport::settlement($num[$valu['price_id']]['num'],$valu['price_id'],$product_id,$is_check);
	...
	*/
	function today_scenic($datetime,$product_id){
		...
		//根据计划汇总
		$plan_fold = ItemReport::plan_fold($list);
		//根据票型汇总
		$ticket_fold = ItemReport::ticket_fold($plan_fold);
		//写入日报表
	...
	*/
	function rakeback($data){
		...
		foreach ($list['channel'] as $key => $value) {
			$rebate[$key] = ItemReport::rakeback_user($value);
		}
		ksort($rebate);
		return $rebate;
	}
	...
	*/
	function topup($data){
    	...
    	foreach ($channel as $k => $v) {
    		$list[$k] = ItemReport::topup_type($k,$v);
    	}*/
    	return $list;
	}
	//分渠道商 按类型汇总
	...
	*/
 	...
 	*@param $price_id 票型ID
 	*@param $discount 结算价格
 	*@param $is_check 核算方式 1 结算价 2 净收入 3分销价
 	*return $money float 金额
 	*/
 	function settlement($nums,$price_id,$product_id,$is_check = 1){
 		$ticketType = F("TicketType".$product_id);
 		/*
 		if(empty($discount)){
 			$discount = $ticketType[$price_id]['discount'];
 		}
 		if(empty($discount) && (int)$is_check === (int)2){
 			$income = $ticketType[$price_id]['income'];
 			if(!empty($income)){
 				$discount = $income
 			}
 		}*///dump($is_check);
 		...
 		$data = [
 			'price'		=>	$ticketType[$price_id]['price'],
 			'rebate'	=>	$ticketType[$price_id]['rebate'],
 			'discount'	=>	$discount,
 			'number'	=>	$nums,
 	...
 	*/
 	...
 	{	
 		if(empty($plan_id)){
 			...
 			$conductor = ItemReport::sum_pay($plan_id,$user_id,'');
 			$collection = ItemReport::collection($plan_id,$user_id,'');
 			$money[$plan_id] = array(
 				'plan'   => $plan_id,
 				'data' => array_merge($conductor,$collection)
 			);
 	...
 	public function months2($datetime='')
 	{
		...
		//}
		$lists = ItemReport::level_fold($list);
