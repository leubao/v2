...
<div class="main row">
  ...
  <div class="col-md-6 mt20">
      <div class="panel panel-warning">
        ...
        <ul class="list-group form-inline">
          ...
          <li class="list-group-item">
            ...
            <select class="form-control" name="cert_type" id="certType">
      ...
      <div class="btn-group">
        <button type="button" id="printActivty" class="btn btn-default" data-toggle="modal"><span class="glyphicon glyphicon-qrcode"></span>立即预约</button>
      </div>
  </div>
  </div>
</div>
...
$(function() {
  ...
  $("#printActivty").bind("click",function(){
    var rstr = "",
      ...
      tour = $("#tourists").val(),
      city = '35',
      number = {$data.param.info.number},//单笔订单最少人数
      activety = {$data.id},
      pre_model = {$pre_model},
      certType = 1,//凭证类型
      id_card = '',//$("#id_card").val(),
    ...
    if(!remark){ remark = "空.."; }
      if(rstr != ""){
        ...
        param = '{"tour":'+tour+',"city":'+city+',"remark":"'+remark+'","id_card":"'+id_card+'","activity":"'+activety+'","cert_type":"'+certType+'","settlement":"'+USER_INFO.group.settlement+'"}';
        var postData = 'info={"subtotal":'+parseFloat($("#subtoal").html())+',"plan_id":'+plan+',"checkin":'+checkinT+',"data":['+ toJSONString + '],"crm":['+crm+'],"param":['+param+']}'; 
        /*提交到服务器*/
        var action = pre_model == 1 ? 'channelBooking' : 'channelPost';
        $.ajax({
          type:'POST',
          url:'index.php?g=Home&m=Order&a='+action+'&act='+type,
          data:postData,
          dataType:'json',
